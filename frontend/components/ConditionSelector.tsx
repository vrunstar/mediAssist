...
export default function ConditionSelector({ selectedConditions, onChange }: ConditionSelectorProps) {
  const t = useTranslations();
  const [isOpen, setIsOpen] = useState(false);
  const [masterList, setMasterList] = useState<MasterListData>({
    common: [
      "Diabetes", "Hypertension", "Asthma", "Hypothyroidism", "GERD",
      "Anxiety", "Depression", "Heart Disease", "Kidney Disease", "Arthritis"
    ],
    categories: {}
  });

  // Pull master conditions list from backend databases
  useEffect(() => {
    api.getConditions()
      .then((data) => {
        if (data && data.common) setMasterList(data);
      })
      .catch((err) => console.error("Error fetching master conditions:", err));
  }, []);
    ...
    <div className="space-y-4">
      ...
      <div>
        <label className="text-sm font-extrabold text-slate-800 dark:text-white flex items-center gap-1.5 uppercase tracking-wide">
          <HeartPulse className="h-4.5 w-4.5 text-primary-500" />
          {t('form.conditions.label')}
        </label>
        <span className="text-xs text-slate-500 dark:text-slate-400 font-medium">
          {t('form.conditions.subtitle')}
        </span>
      </div>
      ...
      {/* 10 Common Conditions + Trigger More */}
      <div className="flex flex-wrap gap-2.5">
        {masterList.common.map((condition) => {
      ...
      <CategoryPopup
        ...
        title={t('title') === "MedAssist — AI चिकित्सा सहायक" ? "सभी बीमारियां खोजें" : "Search All Conditions"}
